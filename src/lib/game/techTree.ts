 ...
 */
export function canBuild(buildingType: string, playerId: string, entities?: Entity[]): boolean {
	const def = getBuildingDef(buildingType)
	if (def.requirements.length === 0) return true

	const resolved =
		entities ?? entityManager.queryEntities(ComponentType.BUILDING, ComponentType.OWNER)
